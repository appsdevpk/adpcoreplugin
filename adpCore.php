...
function adpInitCustomBlocks(){
	$blocksDir = 'blocks';
	$dirPath = get_stylesheet_directory().'/'.APP_DIRECTORY.'/'.$blocksDir.'/';
	
	if(file_exists($dirPath)){
		chdir($dirPath);
		$themeBlocks = glob('*.json');
		if($themeBlocks){
			echo '<pre>';
			foreach($themeBlocks as $tBlock){
				$fileName = basename($tBlock);
				$blockRenderFile = str_ireplace('.json','',$fileName);
				$blockFile = get_stylesheet_directory_uri().'/'.APP_DIRECTORY.'/'.$blocksDir.'/'.$fileName;
				$blockConfig = json_decode(file_get_contents($blockFile),true);
				if(isset($blockConfig['slug']) && isset($blockConfig['title'])){
					$blockAttribs = isset($blockConfig['fields']) ? $blockConfig['fields'] : [];
					$blockConfigOptions = array(
						'title'=>$blockConfig['title'],
						'attributes'=>$blockAttribs,
						'render_callback' => function ( $attributes ) use ($blockRenderFile,$blocksDir) {
							return adpRenderBladeView($blockRenderFile,$blocksDir,[
					...
					//var_dump($blockConfigOptions);
					register_block_type($blockConfig['slug'],$blockConfigOptions);
				}
			}
			echo '</pre>';
		}
		...
		if($themeShortcodesFiles){
			foreach($themeShortcodesFiles as $shortcodeFile){
				...
				add_shortcode($fileName,function($args=array(),$content='',$tag){
					return adpRenderBladeView($tag,'shortcodes',[
						'arguments'=>$args,
						'content'=>$content
					]);
				});
...
include "lib/animations.php";
include "lib/wizards.php";

include "plugin-update-checker/plugin-update-checker.php";
use YahnisElsts\PluginUpdateChecker\v5\PucFactory;

$adpUpdateChecker = PucFactory::buildUpdateChecker(
	'https://github.com/appsdevpk/adpcoreplugin/',
	__FILE__,
	'adpCore'
);

$adpUpdateChecker->setBranch('main');