...
function adpMyLibariesFunc(){
	...
	<table class="wp-list-table widefat fixed striped table-view-list posts">
		<tbody>
		<?php 
		foreach($userLibrary as $ind=>$userLib){ 
			$libKey = base64_encode($userLib);
		?>
			<tr>
				...
				<td style="width:380px">
					...
					<a href="#" class="button includeBtn">Include In</a>
				</td>
			</tr>
			<tr style="display:none">
				<td colspan="2">
					<div id="survey_<?php echo $ind; ?>"></div>
					
					<script>
						var surveyJson_<?php echo $ind; ?> = {
							elements: [{
								"type": "paneldynamic",
								"name": "library-includes-<?php echo $libKey; ?>",
								"title": "Choose one or more include conditions",
								"panelCount": 1,
								"maxPanelCount": 100,
								"confirmDelete": true,
								"templateElements": [{
									...
									"name": "posttype",
									"title": "Post Type",
									"choices": <?php echo json_encode($postTypesOptions); ?>,
									"visibleIf": "{panel.includein} == 'Post Type Is'"
								},{
									"type": "dropdown",
									"name": "taxonomy",
									"title": "Taxonomy",
									"choices": <?php echo json_encode($taxonomiesList); ?>,
									"visibleIf": "{panel.includein} == 'Taxonomy Is'"
								},{
									"type": "text",
									"name": "includeval",
									"startWithNewLine": false,
									"title": "{panel.includein} (separate multiple values with comma, only single value is accepted in function and url)",
									"visibleIf": "{panel.includein} == 'Post ID In' || {panel.includein} == 'Url Is' || {panel.includein} == 'Function Is' || {panel.includein} == 'Terms In'"
								}]
							}]
						};
						var survey_<?php echo $ind; ?> = new Survey.Model(surveyJson_<?php echo $ind; ?>);
						//survey_<?php echo $ind; ?>.data = JSON.stringify(str);
						survey_<?php echo $ind; ?>.completeText = 'Save';
						
						survey_<?php echo $ind; ?>.onComplete.add(function (sender, options) {
							var formData = JSON.stringify(sender.data);
							survey_<?php echo $ind; ?>.clear(false, true);
							survey_<?php echo $ind; ?>.render();
							
							options.showSaveInProgress();
							const xhr = new XMLHttpRequest();
							xhr.open("POST", "<?php echo home_url(); ?>/?adpSaveLibIncludes=1");
						...
						jQuery("#survey_<?php echo $ind; ?>").Survey({ model: survey_<?php echo $ind; ?> });
					</script>
				</td>
			</tr>
		<?php } ?>
	...
	<script>
		...
		jQuery(document).on('click','.includeBtn',function(e){
			e.preventDefault();
...
add_filter( 'query_vars', function($vars){
	...
	$vars[] = "adpSaveLibIncludes";
	
	return $vars;
});
add_action('template_redirect', function($template){
	global $wp_query;
	if(!isset( $wp_query->query['adpSaveLibToMyLib']) && !isset( $wp_query->query['adpRemoveLibFromMyLib']) && !isset( $wp_query->query['adpSaveLibIncludes'])){
		return $template;
	}
	
	if(isset($wp_query->query['adpSaveLibIncludes'])){
		$req = file_get_contents('php://input');
		$data = json_decode($req, true);
		foreach($data as $key=>$val){
			if(substr($key,0,17)=='library-includes-'){
				$keyParts = explode('-',$key);
				$metaKey = 'adpLibIncludes_'.$keyParts[2];
				update_option($metaKey,$req,true);
			}
		}
		exit();
	}
